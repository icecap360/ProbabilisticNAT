...
from libs.nat_misc import NAT_Schedule_MRFSampler, NATSchedule
...
def LSimple(x0, nnet, schedule, sampling_type, **kwargs):
    timesteps, labels, xn = schedule.sample(x0, **kwargs)
    pred = nnet(xn, timesteps=timesteps, **kwargs)  # , **kwargs)
    loss = schedule.loss(pred, labels)
    # masked_token_ratio = (
    #     xn.eq(schedule.mask_ind).sum().item() / xn.shape[0] / xn.shape[1]
    # )
    return loss
...
def train(config, args):
    ...
    )  # accelerate.Accelerator(mixed_precision="bf16")
    ...
    config.searched_strategies_dir = os.path.join(
        args.output_dir, "searched_strategies"
    )
    if accelerator.is_main_process:
    ...
    codebook_size = config.nnet.codebook_size
    config.nnet.codebook_size = codebook_size
    ...
    if args.resume and args.mode == "pretrain":
        state = torch.load(config.resume_path, map_location="cpu")
        train_state.nnet.load_state_dict(state)
        train_state.nnet_ema.load_state_dict(state)
        # train_state.load(config.resume_path, ignored_keys=args.ignored_keys)
        # ckpt_root
    elif args.resume and not bool(os.listdir(config.ckpt_root)):
    ...
    schedule = NAT_Schedule_MRFSampler(
    ...
    def train_step(_batch, accumulation_steps, sampling_type):
        _metrics = dict()
        optimizer.zero_grad()
        ...
        for step in range(accumulation_steps):
            ...
            loss = LSimple(
                _z, nnet, schedule, sampling_type=sampling_type, context=context
            )
    ...
    @torch.no_grad()
    def eval_step(n_samples, sampling_type, **nat_conf):
        ...
        for _batch_size in tqdm(
            utils.amortize(n_samples, batch_size),
            disable=not accelerator.is_main_process,
            desc="sample2dir",
        ):
            contexts = next(test_generator)
            samples = schedule.generate(
                args.gen_steps,
                len(contexts),
                cfg_nnet,
                decode,
    ...
    class DisPatcher:
        def __init__(
            ...
            init_values = {
                "manual_ratios": torch.tensor(
                    np.cos(np.linspace(0, math.pi / 2, args.gen_steps + 1))
                )[1:-1],
                "manual_temp": torch.tensor(1 - np.linspace(0, 1, args.gen_steps + 1))[
                    :-2
                ],
            ...
            self.clip_option = torch.cat(
                [torch.tensor(clip_option[k], dtype=torch.bool) for k in config_set]
            )
            ...
            assert (
                torch.all(cumsum[:-1] <= cumsum[-1])
                and cumsum[-1] == torch.sum(self.clip_option).item()
            )

            logger.info(
                f"self.params: {self.params_clipped}\n{self.params_unclipped}\n{self.params}"
            )
        ...
        def save(self):
            if accelerator.is_main_process:
                save_path = os.path.join(
                    config.searched_strategies_dir, f"stp_{self.upd_idx}.yaml"
                )
        ...
        def update(self):
            ...
            if args.clip_grad is not None:
                self.params_clipped.grad = torch.clamp(
                    self.params_clipped.grad, -args.clip_grad, args.clip_grad
                )
                self.params_unclipped.grad = torch.clamp(
                    self.params_unclipped.grad, -args.clip_grad, args.clip_grad
                )
                logger.info(
                    f"Grad after clipping: {self.params_clipped.grad}\n{self.params_unclipped.grad}"
                )
            self.optimizer.step()  # Update parameters using the optimizer
            logger.info(
                f"updated params: {self.params_clipped}\n{self.params_unclipped}"
            )
            self.params_clipped.data = torch.clamp(self.params_clipped, 0, 0.99).data
            self.params_unclipped.data = torch.clamp(
                self.params_unclipped, min=1e-4
            ).data
            self.upd_idx += 1
            self.base_fid = self.eval(phase=f"stp_{self.upd_idx}", write=True)
            self.save()
        ...
        def params2sd(self, params):
            res = dict(
                zip(
                    self.splits.keys(), torch.split(params, tuple(self.splits.values()))
                )
            )
        ...
        def eval(self, phase, params=None, write=False, sampling_type=None):
            params = self.params if params is None else params
            state_dict = self.params2sd(params)
            res = eval_step(
                n_samples=args.eval_n, sampling_type=sampling_type, **state_dict
            )
            state_dict = {
                k: tuple(v) if len(v.shape) != 0 else v.item()
                for k, v in state_dict.items()
            }
            result = {
                "phase": phase,
            ...
            if accelerator.is_main_process:
                pd.DataFrame(self.history_records).to_csv(
                    os.path.join(args.output_dir, f"dispatcher_{self.record_time}.csv"),
                    index=False,
                )
                if write:
        ...
        def backward(self):
            ...
            for i in range(len(self.params)):
                ...
                fid = self.eval(
                    phase=f"grad_est_{i}_stp_{self.upd_idx}",
                    params=params,
    ...
    if args.mode == "pretrain":
        logger.info(
            f"Start fitting, step={train_state.step}, mixed_precision={accelerator.mixed_precision}"
        )
        now = datetime.datetime.now()
        suffix = now.strftime("%d-%m-%Y-%H-%M-%S")
        exp_name = config.exp_name + f'-{now.strftime("%d-%m-%Y-%H-%M-%S")}'
        if not args.debug and accelerator.is_main_process:
            wandb.init(
                # set the wandb project where this run will be logged
                project="probablistic-nat",
                name=exp_name,
        ...
        while train_state.step < config.train.n_steps:
            ...
            metrics = train_step(
                batch, config.train.accumulation_steps, config.sampling_type
            )
        ...
        if args.mode == "search":
            dispatcher = DisPatcher(
                grad_estimate_step=args.grad_estimate_step_size,
                lr=args.sche_lr,
                unb_lr=args.unb_lr,
            ...
            res = eval_step(
                n_samples=args.eval_n, sampling_type=config.sampling_type, **prev_sd
            )
            if accelerator.is_main_process:
                logger.info(
                    f"Evaluated {args.eval_n} samples with strategy {args.searched_strategy}: {res}"
                )
